...
- **Bot commits to protected `staging`**: `staging.yml`'s `sbom` job and `security.yml`'s ZAP-report step auto-commit straight to `staging` — but `staging` is now branch-protected, and the default `GITHUB_TOKEN` can't push to a protected branch (confirmed: this failed on the first real run with `GH006: Protected branch update failed`). Both steps now detect that rejection and upload the file as a workflow artifact instead of hard-failing, but that means SBOMs/ZAP reports won't land in `docs/security/` automatically until one of: (a) a fine-grained PAT from an account with bypass rights, stored as a secret and used in place of `GITHUB_TOKEN` for just that push, or (b) migrating `staging`'s protection from classic branch protection to a GitHub Ruleset with an explicit bypass entry for GitHub Actions/repo admins. Until then, check the workflow run's artifacts for these files.
