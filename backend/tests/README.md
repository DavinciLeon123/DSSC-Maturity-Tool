...
- `benchmark/`, `perf/` — scoring-engine regression/latency suites (no DB; see CLAUDE.md for how these gate CI).
- `conftest.py` — two independent fixture families: `mami_codes`/`make_answers`
  (no DB) and `postgres_container`/`engine`/`session`/`client`/`admin_client`/
